...
def build_company_detail_page_data(symbol: str) -> dict[str, Any] | None:
    ...
    Returns:
        ...
        otherwise-valid company), and indicators (latest technical
        indicator values, from get_latest_indicator_summary() — all None
        if price_history is empty or too short for a given indicator's
        period).
    """
